...
<div class="mainBodyContent">
    ...
    <div class="mainBodyHeader-responsive row">
        <div class="col-xs-4 yesterdayArrow-responsive text-left">
            <a href="?start={{$previousDay}}&heatType={{$heatType}}&numberOfParticipants={{$numberOfParticipants}}">&#8592; {{$previousDayDisplay}}</a>
        </div>
        <div class="col-xs-4 availableRacesSection">
            <em>{{$strings['str_availableRaces']}}</em>
        </div>
        <div class="col-xs-4 tomorrowArrow-responsive text-right">
    ...
    @if($races !== null)
        ...
        <div class="raceResult" id="{{$race->heatId}}">
            <div class="raceResultHeader row">
                <div class="raceName col-xs-6">{{$race->heatDescription}}</div>
            ...
            <div class="raceResultFooter row">
                <div class="racePrices col-xs-9">{{$numberOfParticipants}} {{$strings['str_drivers']}}
                ...
                <div class="raceBookButtonArea col-xs-3 text-right">
                    @if($authenticated != null)
                        <a href="cart?action=add&heatId={{$race->heatId}}&quantity={{$numberOfParticipants}}"><button type="button" class="formButton formButton-responsive">{{$strings['str_bookIt']}}</button></a>
                    @else
                        <button type="button" class="formButton formButton-responsive" data-toggle="collapse" data-target="#loginOptions_{{$race->heatId}}">{{$strings['str_bookIt']}}</button>
                    @endif
                </div>
            </div>
        </div>
        <div class="loginOptions collapse out centered" id="loginOptions_{{$race->heatId}}">
            ...
            <button type="button" class="regularButton regularButton-responsive" data-toggle="collapse" data-target="#createAccount_{{$race->heatId}}" onclick="$('#loginToAccount_{{$race->heatId}}').collapse('hide')">{{$strings['str_createANewAccount']}}</button>
            ...
            <button type="button" class="regularButton regularButton-responsive" data-toggle="collapse" data-target="#loginToAccount_{{$race->heatId}}" onclick="$('#createAccount_{{$race->heatId}}').collapse('hide')">{{$strings['str_loginToExistingAccount']}}</button>
            ...
            <div class="loginToAccount collapse out" data-toggle="false" id="loginToAccount_{{$race->heatId}}">
                ...
                <form action="login" class="loginForm" method="POST">
                    ...
                    <input type="hidden" name="pageSource" value="step2">
                    <div class="rightAligned">
                        {{link_to('resetpassword',$strings['str_resetPassword'])}} <button type="submit" class="formButton" style="margin-top: 10px; margin-bottom: 10px;">{{$strings['str_login']}}</button>
                    </div>
