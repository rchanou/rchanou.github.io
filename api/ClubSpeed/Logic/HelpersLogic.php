...
class HelpersLogic {
    ...
    public function useSalesTax() {
        $useSalesTax = $this->getControlPanelSettings('MainEngine', array('UseSalesTax'));
        if (isset($useSalesTax) && !empty($useSalesTax) && isset($useSalesTax['UseSalesTax']))
            return \ClubSpeed\Utility\Convert::toBoolean($useSalesTax['UseSalesTax']);
        return true; // default to useSalesTax or default to not (???)
    }
