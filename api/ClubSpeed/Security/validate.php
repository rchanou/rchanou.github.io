...
class Validate {
     ...
     * @return void
     */
    public static function initialize(&$CSLogic) {
        if (!self::$isInitialized) {
            if (!($CSLogic instanceof \ClubSpeed\Business\CSLogic)) {
                $received = gettype($CSLogic);
                if ($received === "object")
                    $received = get_class($CSLogic);
                throw new \InvalidArgumentException("Attempted to initialize Validate class by injecting an object other than a DbConnection! Received: " . $received);
            }
            self::$logic = $CSLogic;
