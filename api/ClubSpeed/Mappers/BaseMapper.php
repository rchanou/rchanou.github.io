...
class BaseMapper {
    ...
    public function mutate() {
        ...
        if (is_int($temp) || is_string($temp)) // will most likely be a string, but needs to be a number
            array_unshift($callbackArgs, \ClubSpeed\Utility\Convert::toNumber($temp));
        if (is_callable($data))
            $data = call_user_func_array($data, $callbackArgs);
        return $this->out($data);
    }

    protected function map($type, $data) {
