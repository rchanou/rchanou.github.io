    ...
    parseString(incomingData, function (err, parsedXml) {
        ...
        switch(command) {
          ...
          case '6':
            ...
            break;
          //case '13': // TIP ADJUSTMENT -- No credit card number, can't handle
            //<XML_REQUEST><USER_ID>User1</USER_ID><COMMAND>13</COMMAND><TROUTD>60005855993</TROUTD><PROCESSOR_ID>VISA</PROCESSOR_ID><MERCH_NUM>999999999911</MERCH_NUM><MANUAL_FLAG>0</MANUAL_FLAG><GRATUITY_AMNT>3.00</GRATUITY_AMNT><MULTI_FLAG>1</MULTI_FLAG></XML_REQUEST>
          case '14': // TIP ADJUSTMENT -- Have to process as a new transaction
            processCharge(parsedXml, function(response) {
              return cb(null, response);
            });
            break;
...
function processCharge(xml, cb) { // CHARGE -- CAPTURED/APPROVED = Success; Anything else is a failure
  ...
  var tipAdjustRequest = '<XML_REQUEST><USER_ID>User1</USER_ID><COMMAND>14</COMMAND><PROCESSOR_ID>VISA</PROCESSOR_ID><MERCH_NUM>999999999911</MERCH_NUM><ACCT_NUM>4242424242424242</ACCT_NUM><EXP_DATE>1018</EXP_DATE><MANUAL_FLAG>0</MANUAL_FLAG><TRANS_AMOUNT>17.00</TRANS_AMOUNT><TICKET_NUM>119974</TICKET_NUM><CARDHOLDER>Sam Tester</CARDHOLDER><CVV2>162</CVV2><GRATUITY_AMNT>3.00</GRATUITY_AMNT><MULTI_FLAG>1</MULTI_FLAG><PRESENT_FLAG>2</PRESENT_FLAG></XML_REQUEST>';
  */
...
/*
// Code from testing...
if(_.has(xml, 'XML_REQUEST.TRACK_DATA[0]')) {
transaction.transactionRequest.payment = {
trackData: {track2: xml.XML_REQUEST.TRACK_DATA[0]}
}
...
deviceType: 5
  ...
  // Use track data, if provided -- verify against Richard's test
  if(_.has(xml, 'XML_REQUEST.TRACK_DATA[0]')) { // TODO verify it comes across but is empty string
      transaction.transactionRequest.payment = {
          trackData: {
              track2: xml.XML_REQUEST.TRACK_DATA[0]
          }
      };
  }
