...
exports.print = function print(input, callback) {
    ...
    /**
     * SCU100 -- Default: 9600 8N1 No Flow Control (In use at SMCSweden)
     */
    ...
    function generateChecksum(data) {
			...
			checksum[0] = (checksumTotal & 0xff) ^ 0xff;
	
			return checksum;
	}

    if(input.printer.type == 'scu100') {
        ...
        printer.on('connect', function(stream) {
            ...
            console.log("\n\n\n" + 'Connected to ' + input.printer.type + ' fiscal device at ' + printerOptions.host + ':' + printerOptions.port);

						console.log('Received this command to print:');
						console.log(input);
						
						carrier = require('carrier');
            var my_carrier = carrier.carry(printer, null, null, /\x10[\x03\x06]+?/);
            my_carrier.on('line',  function(rawData) {
                console.log("\nIncoming message from fiscal:")
								console.log('Response ASCII', rawData);
								console.log('Response Buffer', Buffer(rawData).length);
								console.log('Response Buffer', Buffer(rawData));

                // Reply Data Packet
                if(rawData.length == 96 && rawData[18] == 0x80) {
                    if(rawData[19] != 0x30) {
                        // ERROR
            ...
            setTimeout(function() {
                ...
                receiptData += ('0000000000' + input.check.id).slice(-10);
								receiptData += registerId; // Cash Register ID
								receiptData += ('            ' + input.printer.options.runNumber).slice(-12);
                receiptData += checkType;

                if(input.check.type == 'create') {
                    receiptData += '            0'; // Return Amount
                ...
                var taxArray = [
                    {name: '', percent: '0,00', total: '0', vatId: ''},
                    {name: '', percent: '0,00', total: '0', vatId: ''},
                    {name: '', percent: '0,00', total: '0', vatId: ''},
                    {name: '', percent: '0,00', total: '0', vatId: ''}
                ];
                ...
                var checksum = Buffer(generateChecksum(Buffer.concat([Buffer(registerId), command, receiptData])));

                var packet = Buffer.concat([start, Buffer(registerId), command, receiptData, end, checksum]);
                ...
                console.log('Packet Length ' + packet.length);
								if(packet.length !== 171) {
									console.log('Invalid packet length (' + packet.length + ' bytes), should be 171 bytes');
								}
    ...
    /**
     * CLEANCASH (In use at GKCStockholm)
     */
    ...
    if(input.printer.type == 'cleancash') {
        ...
        printer.on('connect', function(stream) {
            ...
            setTimeout(function() {
                ...
                var length = ("000" + (command.length + 3).toString(16).toUpperCase()).slice(-3);
                command = Buffer(command.replace('LEN', length));
                command = Buffer.concat([command, Buffer(("00" + calculateChecksum(command, 0)).slice(-2)), Buffer([0x0d])]);
    ...
    if(input.printer.type == 'posnet') { // (In use at GKCPoland)
