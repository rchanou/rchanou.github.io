...
Ingenico.prototype.submitTransaction = function submitTransaction(order, creditCard, prospect, other) {
	...
	var usingToken = (creditCard && creditCard.TRANSARMORTOKEN && creditCard.CCEXP && creditCard.TRANSARMORTOKENPROVIDERID);

	if(usingToken) {
		req = '<TRANSACTION>' +
			'  <TRANSACTIONTYPE>CCAUTH</TRANSACTIONTYPE>' +
			'￼￼<TRANSARMORTOKEN>' + creditCard.TRANSARMORTOKEN + '</TRANSARMORTOKEN>' +
			'￼￼<CCAMT>' + convertToDecimal(order.amount) + '</CCAMT>' +
			'￼￼<CCEXP>' + creditCard.CCEXP + '</CCEXP>' +
			'￼￼<TRANSARMORTOKENPROVIDERID>' + creditCard.TRANSARMORTOKENPROVIDERID + '</TRANSARMORTOKENPROVIDERID>' +
			'</TRANSACTION>';
		...
		var hoistedVars = {
				...
				_original: result,
				// Might need to coerce the tokens into the proper, normalized format?
			};

		// Really will do the whole process
		return new Promise(function(resolve, reject) {
			resolve(_.merge(self.opts.defaultHoistedVars, {
							success: true,
							}, hoistedVars));
		});
	} else {
...
Ingenico.prototype.tokenize = function tokenize(order, creditCard, prospect, other) {
	var self = this;
	var req = '';

	req = '<TRANSACTION>' +
		'  <TRANSACTIONTYPE>INTERACTIVETATOKENIZE</TRANSACTIONTYPE>' +
		'￼￼<SERVICENAME>CARD</SERVICENAME>' +
		'￼￼<TRANIDENT>TATest</TRANIDENT>' +
		'</TRANSACTION>';
	...
	return new Promise(function(resolve, reject) {
		resolve(_.merge(self.opts.defaultHoistedVars, {
						...
						additionalTokenInformation: {
							'TRANSARMORTOKEN': 'ABC123',
							'CCEXP': '0118',
							'TRANSARMORTOKENPROVIDERID': '001'
							},
						_original: {}
						}));
	});
...
Ingenico.prototype.refundTransaction = function refundTransaction(order, creditCard, prospect, other) {
	var self = this;
	var req = '';
	...
	if(usingToken) {
    	...
    	'  <CCAMT>16.95</CCAMT>' +
    	'  <TRANSARMORTOKEN>xxxxxxxxxxxx2229</TRANSARMORTOKEN>' +
    	'  <CCEXP>xxxx</CCEXP>' +
    	'  <TRANSARMORTOKENPROVIDERID>001</ TRANSARMORTOKENPROVIDERID>' +
    	'</TRANSACTION>';
