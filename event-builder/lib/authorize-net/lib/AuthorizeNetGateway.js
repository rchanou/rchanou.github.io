    ...
    return toJson(body)
      .then(function (result) {
        ...
        if (json.ErrorResponse) {
					if(json.ErrorResponse.length >= 1) {
  ...
  return sendXmlifiedRequest(this, obj)
    .spread(createJsonCallback(function (json) {
        ...
        if (json.createTransactionResponse.messages[0].resultCode[0] === 'Error') {
          if(!json.createTransactionResponse.transactionResponse[0].hasOwnProperty('errors')) {
						throw new GatewayError(json.createTransactionResponse.messages[0].message[0].text[0], json.createTransactionResponse);
					} else {
...
AuthorizeNetGateway.prototype.submitTransaction = function submitTransaction (order, creditCard, prospect, other) {
  ...
  var body = {
	...
	if(typeof creditCard.track1 !== 'undefined') {
		body.payment.trackData = {
				track1: creditCard.track1
		};		
	} else if(typeof creditCard.track2 !== 'undefined') {
		body.payment.trackData = {
				track2: creditCard.track2
		};
	} else {
  	...
  	var expirationMonth = creditCard.expirationMonth.toString().length === 2 ? creditCard.expirationMonth.toString() : '0' + creditCard.expirationMonth.toString();
		...
		if(creditCard.cvv && creditCard.cvv.length > 0) body.payment.creditCard.cardCode = creditCard.cvv;
	}
  ...
  if (prospect) {
	...
	body.retail = {
			marketType: other.marketType || 2,
			deviceType: other.deviceType || 5
		};
...
AuthorizeNetGateway.prototype.createCustomerProfile = function (payment, billing, shipping, options) {
  ...
  var body = {
	...
	if(payment.cvv && payment.cvv.length > 0) body.payment.creditCard.cardCode = payment.cvv;
...
AuthorizeNetGateway.prototype.createSubscription = function createSubscription (cc, prospect, subscriptionPlan, other) {
  ...
  var body = {
	...
	if(cc.cvv && cc.cvv.length > 0) body.payment.creditCard.cardCode = cc.cvv;
